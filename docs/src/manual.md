...
Manual coin selection allows you to explicitly choose which UTXOs (unspent transaction outputs) from your wallet to use when building a transaction. By default, LWK greedily adds the largest UTXOs until the transaction amount and fees are covered. However, manual coin selection gives you control over which specific UTXOs are spent, which can be useful for privacy, UTXO management, or specific transaction requirements.
...
```typescript
{{#include ../../lwk_wasm/tests/node/manual_coin_selection.js:get_utxos}}
```
...
```typescript
{{#include ../../lwk_wasm/tests/node/manual_coin_selection.js:manual_coin_selection}}
```
