...
pub struct WaterfallsClient {
    inner: Arc<lwk_wollet::asyncr::WaterfallsClient>,
    network: Network,
}

impl WaterfallsClient {
    pub fn from_client(
        client: Arc<lwk_wollet::asyncr::WaterfallsClient>,
        network: Network,
    ) -> Self {
        Self {
            inner: client,
    ...
    pub fn new(url: &str, network: Network) -> Self {
        Self {
            inner: Arc::new(lwk_wollet::asyncr::WaterfallsClient::new(network, url)),
            network,
        }
    }
}
...
impl boltz_client::network::LiquidClient for WaterfallsClient {
