...
pub use crate::invoice_enum::Invoice;
...
impl BoltzSession {
    ...
    async fn fetch_bolt12_invoice(
        ...
        log::error!("offer: {}\ninvoice: {}", offer, response.invoice);
...
fn bitcoin_chain_from_network(network: Network) -> BitcoinChain {
    match network {
        ...
        Network::CustomElements { .. } => BitcoinChain::BitcoinRegtest,
    }
}
...
pub fn elements_network_to_liquid_chain(network: Network) -> LiquidChain {
    match network {
        ...
        Network::CustomElements { .. } => LiquidChain::LiquidRegtest,
    }
}
...
pub fn boltz_default_url(network: Network) -> &'static str {
    match network {
        ...
        Network::CustomElements { .. } => BOLTZ_REGTEST,
    }
}
...
mod tests {
    ...
    fn test_elements_network_to_liquid_chain() {
        ...
        let networks = vec![
            lwk_wollet::Network::Liquid,
            lwk_wollet::Network::TestnetLiquid,
            lwk_wollet::Network::default_regtest(),
        ];
    ...
    #[test]
    fn test_verify_invoice_from_offer_1() {
        let offer_str = "lno1qgsqvgnwgcg35z6ee2h3yczraddm72xrfua9uve2rlrm9deu7xyfzrckyypvfud3qww60z9k4p60hhmwlf6zjk20pu93hqw6pse60yqtkypnqmc";
        ...
        let invoice = crate::parse_bolt12_invoice(invoice_str).unwrap();

        assert!(crate::verify_invoice_from_offer(&invoice, &offer));
    }

    #[test]
    fn test_verify_invoice_from_offer_2() {
        let offer_str = "";
