...
They include e2e flows that simulate usage from CLI users.
```sh
cargo test
```

Tests with authenticated backends require close source image, if you have access to it, run them with
```sh
cargo test auth -- --include-ignored
```
