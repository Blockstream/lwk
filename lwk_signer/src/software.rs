...
use elements_miniscript::{
    ...
    elements::{
        ...
        hashes::Hash,
        pset::{self, Input as PsetInput, PartiallySignedTransaction},
        schnorr::TapTweak,
        secp256k1_zkp::All,
        sighash::{self, SchnorrSighashType, SighashCache},
        taproot::TapLeafHash,
...
struct InputSignData {
    ...
    tap_script_msgs: BTreeMap<TapLeafHash, Message>,
}

/// Sign PSET input using ECDSA
fn sign_pset_ecdsa<C: secp256k1::Signing + secp256k1::Verification>(
    ...
    // (Optional) Verify the signature before inserting
    if secp.verify_ecdsa(msg, &signature, &pubkey.inner).is_err() {
        return false;
    }

    // Using your existing elementssig_to_rawsig formatter
    let raw_sig = elementssig_to_rawsig(&(signature, hash_ty));
    pset_input.partial_sigs.insert(pubkey, raw_sig).is_none()
}

/// Sign PSET input using Schnorr
...
impl SwSigner {
    ...
    /// Set specific network for signer.
                ...
                if fp == &signer_fingerprint {
                    if leaf_hashes.is_empty() {
                        ...
                        for lh in leaf_hashes {
                            if let Ok(PsbtSighashMsg::TapSighash(m)) =
                                pset.sighash_msg(i, &mut sighash_cache, Some(*lh), genesis_hash)
                            {
                                sign_data.tap_script_msgs.insert(
                                    *lh,
                                    Message::from_digest_slice(&m.to_byte_array())
                                        .expect("valid length"),
                                );
        ...
        for (input, sign_data) in pset.inputs_mut().iter_mut().zip(messages) {
                ...
                input.tap_key_origins.clone().iter()
            {
                if fp == &signer_fingerprint {
                    ...
                    if leaf_hashes.is_empty() {
                        if let Some(msg) = sign_data.tap_key_msg {
                        ...
                        for leaf_hash in leaf_hashes {
                            if let Some(msg) = sign_data.tap_script_msgs.get(leaf_hash) {
                                if sign_pset_schnorr(
                                    &self.secp,
                                    secret_key,
                                    *xonly,
