...
//!
//! Runtime witness directives are resolved against signer capabilities provided
//! by [`SignerMeta`].
...
pub fn resolve_witness<Signer: SignerMeta>(
    bytes: &[u8],
    signer_meta: &Signer,
        ...
        match value {
            RuntimeSimfWitness::SigHashAll { name, public_key } => {
                ...
                final_witness.insert(
                    ...
                    Value::byte_array(
                        signer_meta
                            .sign_schnorr(sighash_all, signer_public_key)?
                            .serialize(),
                    ),
                );
            }
