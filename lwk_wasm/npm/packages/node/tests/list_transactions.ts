...
export default async function runListTransactionsTest() {
    try {
        ...
        const balance_utxo_only = wollet_utxo_only.balance().entries();

        const lbtc = network.policyAsset().toString();
        const lbtc_balance = balance.get(lbtc);
        const lbtc_utxo_only_balance = balance_utxo_only.get(lbtc);
        console.assert(lbtc_balance === lbtc_utxo_only_balance);

        // Remove L-BTC from balances for comparison
        balance.delete(lbtc);
        balance_utxo_only.delete(lbtc);

        function serializeBalance(balance: lwk.Balance): string {
            return JSON.stringify([...balance.entries()], (_key, value) =>
              typeof value === "bigint" ? value.toString() : value
            );
        }

        // Compare remaining balances
        console.assert(serializeBalance(balance) === serializeBalance(balance_utxo_only));
