...
async function runListTransactionsTest() {
    try {
        ...
        assert.equal(desc.toString(),  "ct(slip77(9c8e4f05c7711a98c838be228bcb84924d4570ca53f35fa1c793e58841d47023),elwpkh([73c5da0a/84'/1'/0']tpubDC8msFGeGuwnKG9Upg7DM2b4DaRqg3CUZa5g8v2SRQ6K4NSkxUgd7HsL2XVWbVm39yBA4LAxysQAm397zwQSQoQgewGiYZqrA9DsP4zbQ1M/<0;1>/*))#2e4n992d");
        ...
        assert(txs.length >= 20);
        const opt = lwk.TxsOpt.default();
        const txs2 = wollet.txs(opt);
        assert.equal(txs.length, txs2.length);
        const num_txs = wollet.numTxs();
        assert.equal(txs.length, num_txs);

        const limit = 10;
        const page1 = wollet.transactionsPaginated(0, limit);
        assert.equal(page1.length, limit);
        assert.equal(JSON.stringify(page1.map(t => t.txid)), JSON.stringify(txs.slice(0, limit).map(t => t.txid)));

        const offset = 15;
        const page2 = wollet.transactionsPaginated(offset, limit);
        assert.equal(page2.length, limit);
        assert.equal(JSON.stringify(page2.map(t => t.txid)), JSON.stringify(txs.slice(offset, offset + limit).map(t => t.txid)));

        const page1_opt = lwk.TxsOpt.withPagination(0, limit);
        const page1_txs = wollet.txs(page1_opt);
        assert.equal(page1_txs.length, limit);
        assert.equal(JSON.stringify(page1_txs.map(t => t.txid)), JSON.stringify(txs2.slice(0, limit).map(t => t.txid)));

        const page2_opt = lwk.TxsOpt.withPagination(offset, limit);
        const page2_txs = wollet.txs(page2_opt);
        assert.equal(page2.length, limit);
        assert.equal(JSON.stringify(page2_txs.map(t => t.txid)), JSON.stringify(txs2.slice(offset, offset + limit).map(t => t.txid)));
        ...
        assert(txs_utxo_only.length < txs.length);
        ...
        assert.equal(lbtc_balance, lbtc_utxo_only_balance);
        ...
        assert.equal(JSON.stringify(balance), JSON.stringify(balance_utxo_only));
