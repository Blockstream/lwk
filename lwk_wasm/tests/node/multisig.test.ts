...
describe("Multisig", function () {
  it("should perform multisig wallet operations", async function () {
    ...
    const mnemonic_a = new lwk.Mnemonic(
    ...
    // Fixed mnemonic with some funds // ANCHOR: ignore
    const mnemonic_b = new lwk.Mnemonic(
    ...
    // Fixed mnemonic with some funds // ANCHOR: ignore
    const mnemonic_c = new lwk.Mnemonic(
    ...
    let pset = builder.finish(wollet_c);
    ...
    expect(details.balance().fee()).to.be.lessThan(100);
    // PSET has a signature from Carol
    expect(details.fingerprintsHas()).to.have.lengthOf(1);
    expect(details.fingerprintsHas()).to.include(signer_c.fingerprint());
    // PSET needs a signature from either Bob or Carol
    expect(details.fingerprintsMissing()).to.have.lengthOf(2);
    expect(details.fingerprintsMissing()).to.include(signer_a.fingerprint());
    expect(details.fingerprintsMissing()).to.include(signer_b.fingerprint());
    // PSET has a single recipient, with data matching what was specified above
    expect(details.balance().recipients()).to.have.lengthOf(1);
    const recipient = details.balance().recipients()[0];
    expect(recipient.address().toString()).to.equal(address.toString());
    expect(recipient.asset().toString()).to.equal(asset.toString());
    expect(recipient.value()).to.equal(sats);
    ...
    expect(details_b.fingerprintsHas()).to.have.lengthOf(2);
