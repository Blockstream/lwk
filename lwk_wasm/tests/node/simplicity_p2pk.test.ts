...
describe("SimplicityP2pk", function () {
  it("should handle Simplicity P2PK operations", async function () {
    const network = lwk.Network.testnet();
    const genesisHash = network.genesisBlockHash();

    expect(genesisHash, "Genesis hash mismatch").to.equal(
      "a771da8e52ee6ad581ed1e9a99825e5b3b7992225534eaa2ae23244fe26ab1c1"
    );
    expect(genesisHash.length, "Genesis hash length should be 64").to.equal(64);
    ...
    expect(cmr, "CMR mismatch").to.equal(TEST_CMR);
    ...
    expect(address.toString(), "Address mismatch").to.equal(TEST_ADDRESS);
    ...
    expect(utxo.value(), "UTXO value mismatch").to.equal(TEST_UTXO_VALUE);
    ...
    expect(finalizedTx, "Finalized transaction should not be null").to.not.be
      .null;
    const finalizedHex = finalizedTx.toString();
    expect(finalizedHex, "Finalized transaction mismatch").to.equal(
      TEST_FINALIZED_TX
    );
    ...
    const _tTuple = lwk.SimplicityType.fromElements([
      lwk.SimplicityType.u32(),
    ...
    const _vLeft = lwk.SimplicityTypedValue.left(
      lwk.SimplicityTypedValue.fromU32(42),
      lwk.SimplicityType.boolean()
    );
    const _vRight = lwk.SimplicityTypedValue.right(
      lwk.SimplicityType.u32(),
      lwk.SimplicityTypedValue.fromBoolean(true)
    );
    const _vTuple = lwk.SimplicityTypedValue.fromElements([
      lwk.SimplicityTypedValue.fromU32(42),
      lwk.SimplicityTypedValue.fromU256Hex(TEST_PUBLIC_KEY),
    ]);
    const _vNone = lwk.SimplicityTypedValue.none(lwk.SimplicityType.u64());
    const _vSome = lwk.SimplicityTypedValue.some(
      lwk.SimplicityTypedValue.fromU64(BigInt(1000))
    );
    ...
    expect(args2, "args2.addValue result should not be null").to.not.be.null;
    ...
    expect(witness2, "witness2.addValue result should not be null").to.not.be
      .null;
    ...
    expect(lwk.bytesToHex(program2.cmr()), "Program2 CMR mismatch").to.equal(
      TEST_CMR
    );
  });
});
