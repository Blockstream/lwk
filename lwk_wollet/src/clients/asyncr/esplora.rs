...
#[cfg(not(target_arch = "wasm32"))]
pub use waterfalls::{WaterfallsReconnectingSubscription, WaterfallsSubscription};
...
const WATERFALLS_MAX_ADDRESSES_REQUESTS: usize = 1_000;
...
impl EsploraClient {
    ...
    async fn get_address_history_waterfalls_from_page(
        ...
        for _ in 0..WATERFALLS_MAX_ADDRESSES_REQUESTS {
            let url = format!(
                "{}/v2/waterfalls?addresses={}&page={}",
            ...
            let waterfalls_result: WaterfallsResult = serde_json::from_str(&body)?;
            let has_more = waterfalls_result
                .has_more
                .as_ref()
                .is_some_and(|has_more| has_more.iter().any(|addr| addr == address));

            if let Some(chain_history) = waterfalls_result.txs_seen.get("addresses") {
                if let Some(history) = chain_history.first() {
        ...
        Err(Error::Generic("too many pages requests".into()))
    }
...
mod tests {
    ...
    use crate::{
        asyncr::{EsploraClientBuilder, WaterfallsClientBuilder},
        clients::{asyncr::async_sleep, oauth::fetch_oauth_token, Data, History, TokenProvider},
    ...
    async fn waterfalls_descriptor_is_encrypted_and_cached() {
        ...
        let mut client = WaterfallsClientBuilder::new("http://example.com", Network::Liquid)
            .build()
            .unwrap();
