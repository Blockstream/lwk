...
fn lbtc() -> (AssetId, RegistryData) {
    let asset_id = *Network::Liquid.policy_asset();
...
fn tlbtc() -> (AssetId, RegistryData) {
    let asset_id = *Network::TestnetLiquid.policy_asset();
    let data = RegistryData {
        contract: Contract {
            entity: Entity::Domain("".to_string()),
            issuer_pubkey: vec![2; 33],
...
fn rlbtc() -> (AssetId, RegistryData) {
    let asset_id = *Network::default_regtest().policy_asset();
    let data = RegistryData {
        contract: Contract {
            entity: Entity::Domain("".to_string()),
            issuer_pubkey: vec![2; 33],
...
mod tests {
    ...
    fn test_registry_cache_hardcoded() {
        ...
        let regtest_asset_id = *Network::default_regtest().policy_asset();
        let testnet_asset_id = *Network::TestnetLiquid.policy_asset();
        let mainnet_asset_id = *Network::Liquid.policy_asset();
        let usdt_asset_id =
            AssetId::from_str("ce091c998b83c78bb71a632313ba3760f1763d9cfcffae02258ffa9865a37bd2")
                .unwrap();
