...
/// against Keycloak's JWKS + credit checking). Both static-token auth and the automatic
/// OAuth refresh (`TokenProvider::Blockstream`) share one auth stack here, since bringing it
/// up is slow (Leo's ask to not duplicate setup):
/// - a valid static token is served, bogus/missing tokens and exhausted credits are denied
///   with the proxy's JSON-RPC codes;
/// - a `Blockstream` token is minted at build and, once it expires, transparently refreshed
///   on the next reconnection.
///
/// Transport difference with `test_esplora_token_expiry_authenticated`: the proxy validates
/// the token only on each connection's first message and closes denied connections, so an
/// established connection keeps being served after its token expires; the expired token
/// surfaces (as -32004) only on a new connection, exercised here by restarting the proxy.
...
fn test_electrum_authenticated() {
    ...
    // Polling instead of a fixed sleep keeps the test honest about the proxy's `exp` leeway
    // (the jsonwebtoken crate defaults to 60s, so the denial arrives ~65s after the mint).
    ...
    // The established connection is still served with the expired token: the proxy validates
    // per connection, not per message.
    assert_eq!(client.tip().unwrap().height, 102);

    // Drop the authenticated connection (as a production restart or idle disconnect would):
    // the next call reconnects, gets denied with -32004 because the cached token expired, and
    // transparently retries with a freshly minted token.
